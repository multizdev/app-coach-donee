...
function ReScheduleTimeChip({ item: { time } }: { item: Time }): ReactElement {
  const { selectedDate, selectedDates, addSelectedDate } = useRescheduleStore();
...
function TimeNotAvailable() {
  const { selectedDay } = useRescheduleStore();
...
export { ReScheduleTimeChip, TimeNotAvailable };
